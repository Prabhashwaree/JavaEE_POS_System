...
$("#cmbItemCode").click(function () {
    ...
    $.ajax({
        ...
        success(responce) {
            for (var i of responce.data) {
                if (itemsCode == i.itemCode) {
                    $("#txtNameItem").val(i.itemName);
                    $("#txtPriceItem").val(i.price);
                    $("#txtQtyHandItem").val(i.qty);
                }
            }
        }
    })
...
function addToCart() {
    ...
    for (var i in crats) {
        if (crats[i].itemCode == itemCode1) {
            var newQty = +crats[i].qty + +orderQty1;
            var newTotal = itemPrice1 * newQty;
            crats[i].qty(newQty);
            crats[i].total(newTotal);
            return;
        }
    }
...
function loadAllCartTable() {
    ...
    for (var i of crats) {
        ...
        let row = `<tr><td>${i.itemCode1}</td><td>${i.itName1}</td><td>${i.orderQty1}</td><td>${i.itemPrice1}</td><td>${i.total2}</td>
...
function saveConfirmOrder() {
    ...
    $.ajax({
        ...
        success: function (addAll) {
            if (addAll.status == 200) {
                genarateOrderId();
                alert(addAll.data);
...
function  lordOrderDetails() {
    $("#orderDetailsTable").empty();
    $.ajax({
        url: "http://localhost:8080/pos/orderDetails",
        method: "GET",
        success(responce1) {
            for (const i of responce1.data) {
                let row = `<tr>
              <td>${i.oId}</td>
              <td>${i.iCode}</td>
              <td>${i.oQTY}</td>
